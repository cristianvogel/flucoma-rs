...
pub fn kmeans_fit(
    ...
    unsafe {
        cpp!([
            ...
            input as "const double*",
            rows as "ptrdiff_t", cols as "ptrdiff_t",
            k as "ptrdiff_t", max_iter as "ptrdiff_t",
            init_method as "ptrdiff_t", seed as "ptrdiff_t",
            means_out as "double*",
...
pub fn grid_process(
    ...
    unsafe {
        cpp!([
            ...
            for (ptrdiff_t r = 0; r < rows; ++r) {
                RealVector p(2);
                if (!result.get(std::to_string(r), p)) return false;
                output[r * 2] = p(0);
                output[r * 2 + 1] = p(1);
            }
            return true;
...
pub fn dataset_query_process(
    ...
    unsafe {
        cpp!([
            input as "const double*",
            rows as "ptrdiff_t", cols as "ptrdiff_t",
            selected_cols as "const ptrdiff_t*", selected_count as "ptrdiff_t",
            cond_cols as "const ptrdiff_t*", cond_ops as "const ptrdiff_t*",
            cond_vals as "const double*", cond_and_flags as "const ptrdiff_t*",
            ...
            for (ptrdiff_t r = 0; r < rows; ++r) {
                RealVector p(cols);
                for (ptrdiff_t c = 0; c < cols; ++c) p(c) = input[r * cols + c];
                in_ds.add(std::to_string(r), p);
            }

            DataSetQuery q;
            for (ptrdiff_t i = 0; i < selected_count; ++i) q.addColumn(selected_cols[i]);
            ...
            for (ptrdiff_t i = 0; i < cond_count; ++i) {
                ...
                if (!q.addCondition(cond_cols[i], op_str(cond_ops[i]), cond_vals[i], conjunction))
                    return false;
            }
            ...
            q.process(in_ds, current, out_ds);
