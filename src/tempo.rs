use crate::analyzation::OnsetDetectionFunctions;
use crate::segmentation::OnsetSegmentation;
...
impl TempoEstimator {
    ...
    fn segment_channel_onsets(
        ...
        let mut seg = OnsetSegmentation::new(
    ...
    fn compute_novelty(&self, channel: &[f64], n_hops: usize) -> Result<Vec<f64>, &'static str> {
        let mut odf = OnsetDetectionFunctions::new(
            self.config.window_size,
            self.config.fft_size,
            self.config.filter_size,
        )?;
        ...
        for h in 0..n_hops {
            let start = h * self.config.hop_size;
            let frame = &channel[start..start + self.config.window_size];
            let val = odf.process_frame(frame, self.config.function, self.config.filter_size, 0);
...
mod tests {
    ...
    #[test]
    fn tempo_estimation_on_real_drum_loop_115_bpm() {
        let mut path = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        path.push("resources/audio_files/RN_808Beat_115.wav");
        let path_str = path.to_string_lossy();

        let (samples, sample_rate, n_channels, n_frames) =
            read_wav_channel_major(&path_str).expect("failed to read test loop");

        let estimator = TempoEstimator::new(TempoConfig::default());
        let estimated_bpm = estimator
            .estimate(&samples, n_frames, n_channels as usize, sample_rate as f64)
            .expect("tempo estimation should succeed on drum loop");

        assert!(
            (estimated_bpm - 115.0).abs() < 5.0,
            "Expected ~115 BPM, got {}",
            estimated_bpm
        );
    }

    fn read_wav_channel_major(path: &str) -> Result<(Vec<f64>, u32, u16, usize), &'static str> {
        let mut wav = Wav::<f32>::from_path(path).map_err(|_| "failed to open wav")?;
        ...
        for frame in wav.frames() {
            for &sample in frame.iter() {
                interleaved.push(sample as f64);
            }
        }
